...
const uploadImage = async (file) => {
  const r = ref(storage, `publicaciones/${Date.now()}_${file.name}`);
...
export default function AdminPublicacionesPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setItems(pubSnap.docs.map(d => ({ id: d.id, ...d.data() })));
      setEspecialidades(espSnap.docs.map(d => ({ id: d.id, ...d.data() })));
    } catch (e) {
  ...
  const moveItem = async (item, dir) => {
    ...
    await Promise.all([
      updateDoc(doc(db, "publicaciones", a.id), { orden: b.orden, updatedAt: serverTimestamp() }),
      updateDoc(doc(db, "publicaciones", b.id), { orden: a.orden, updatedAt: serverTimestamp() }),
    ]);
    fetchData();
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        await updateDoc(doc(db, "publicaciones", editing.id), { ...payload, updatedAt: serverTimestamp() });
        Swal.fire({ icon: "success", title: "Publicación actualizada", timer: 1200, showConfirmButton: false });
      } else {
        await addDoc(collection(db, "publicaciones"), { ...payload, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
        Swal.fire({ icon: "success", title: "Publicación creada", timer: 1200, showConfirmButton: false });
      }
  ...
  const handleToggle = async (item) => {
    await updateDoc(doc(db, "publicaciones", item.id), { activo: !item.activo, updatedAt: serverTimestamp() });
    fetchData();
  };
  ...
  const handleDelete = async (item) => {
    ...
    if (result.isConfirmed) {
      try {
        await deleteDoc(doc(db, "publicaciones", item.id));
  ...
  const getEspNombre = (id) => especialidades.find(e => e.id === id)?.titulo || "—";
    ...
    <div>
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-3 mb-1">
              ...
              <h1 className="font-sans text-3xl font-bold text-primary">Publicaciones</h1>
            </div>
            <p className="mt-1 ml-4 font-sans text-xs text-text/50 uppercase tracking-wider">
              Artículos, guías y contenido del estudio
            </p>
          </div>
          <motion.button
      ...
      {loading ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          ...
          <p className="font-sans text-sm font-bold uppercase tracking-widest text-text/50">No hay publicaciones</p>
          <p className="mt-2 font-sans text-xs text-text/40">Crea la primera publicación para que aparezca en el sitio.</p>
        </motion.div>
      ) : (
                ...
                <div className="relative h-40 sm:h-auto sm:w-48 flex-shrink-0 overflow-hidden bg-bg-alt">
                  {item.imagen ? (
                    ...
                    <div className="flex h-full min-h-[160px] items-center justify-center">
                      <FileText size={32} className="text-text/20" />
                    </div>
                  )}
                  ...
                  <div>
                    ...
                    {item.especialidad && (
                      <span className="inline-block rounded-full bg-accent/10 px-2.5 py-0.5 font-sans text-[8px] font-semibold uppercase tracking-wider text-accent mb-2">
                        {getEspNombre(item.especialidad)}
                      </span>
                    )}
                    {item.descripcion && (
                      <p className="font-sans text-xs text-text/60 line-clamp-2 mt-1">{item.descripcion}</p>
                    )}
                  </div>
                  ...
                  <div className="mt-4 space-y-4">
                    <div>
                      ...
                      <input type="text" value={form.titulo}
                        ...
                        placeholder="Ej: Nueva regulación ambiental en el Perú"
                    ...
                    <div>
                      ...
                      <textarea value={form.descripcion}
                        ...
                        placeholder="Breve resumen que aparece en la lista de publicaciones"
                        rows={2}
                    ...
                    <div>
                      <label className="mb-1.5 block font-sans text-[10px] font-semibold uppercase tracking-wider text-text/60">Especialidad relacionada</label>
                      ...
                      <p className="mt-1 font-sans text-[9px] text-text/40">La publicación aparecerá en la sección de la especialidad seleccionada.</p>
                    </div>
                  </div>
                </div>
                    ...
                    <label className={`flex cursor-pointer items-center gap-4 rounded-sm border-2 p-4 transition-all ${
                      ...
                      <div>
                        <p className={`font-sans text-[9px] font-semibold uppercase tracking-wider ${form.activo ? "text-green-700" : "text-text/50"}`}>
                          {form.activo ? "Publicación Activa" : "Publicación Oculta"}
                        </p>
